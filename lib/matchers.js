...
const matchers = [
    ...
    },
    {
        ...
        handler(options) {
            ...
            if (options.path.startsWith(FB_PATH.STORY_PHP)) {
                const storyId = options.definition.searchParams.get('story_fbid');
                if (storyId) {
                    story = options.identify(storyId);
                    matched = true;
                }
            ...
            return { matched, story, page, user };
        },
    },
    {
        name: 'aStoryAsPermalinkPhp',
        keys: ['story', 'page', 'user', 'comment'],
        handler(options) {
            let matched = false;
            let story;
            let page;
            ...
            if (options.path.startsWith(FB_PATH.PERMALINK_PHP)) {
                matched = true;
                ...
                const commentId = options.definition.searchParams.get('comment_id');
                if (id) {
                    comment = options.identify(commentId);
                }
            }

            return { matched, story, page, user, comment };
        },
    },
    {
        name: 'aNote',
    ...
    {
        name: 'aUserAsProfilePhp',
        keys: ['user'],
        handler(options) {
            let matched = false;
            let user;

            if (options.path.startsWith(FB_PATH.PROFILE_PHP)) {
                const id = options.definition.searchParams.get('id');
                if (id) {
                    user = options.identify(id);
                    matched = true;
