...
const pathNotStartingByAnyKnownPath = (path) => (
    Object.values(FB_PATH).every((fbPath) => {
        // Check its not a known url
        return !(path.toLowerCase().startsWith(fbPath) && path.length < (fbPath.length + 1));
    })
...
const hasSegment = (segment, segments) => !!segments.find((s) => s.toLowerCase() === segment.slice(1).toLowerCase());
